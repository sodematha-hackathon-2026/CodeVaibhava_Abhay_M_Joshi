...
import { SocialLinkAdminItem, SocialLinkUpsert } from "@/types";
...
export const SocialPage: React.FC = () => {
  ...
  const handleOpenDialog = (item?: SocialLinkAdminItem) => {
    if (item) {
      setEditingItem(item);
      setFormData({
      ...
      setEditingItem(null);
      setFormData({ platform: "", url: "", active: true });
    }
    setDialogOpen(true);
    setMessage("");
    setError("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await socialService.update(editingItem.id, formData);
  ...
  const columns: Column<SocialLinkAdminItem>[] = [
