...
import { enablePush, disablePush, loadPushEnabled } from "../services/pushService";
...
export default function EventCalendarScreen() {
  ...
  useEffect(() => {
    loadPushEnabled().then(setNotifEnabled).catch(() => {});
  }, []);
  ...
  const toggleNotifications = async () => {
    ...
    try {
      if (notifEnabled) {
        ...
        Alert.alert(t('devoteeRegistration.success'), "Push notifications disabled");
      } else {
        await enablePush();
        setNotifEnabled(true);
        Alert.alert(t('devoteeRegistration.success'), "Push notifications enabled");
      }
    } catch (e: any) {
      console.log("Push toggle error:", e?.message || e);
      Alert.alert(t('errors.error'), e?.message || "Failed to update notifications");
    } finally {
        ...
        <View style={{
          ...
          >
            <Text style={{
              ...
              {notifLoading ? "..." : notifEnabled ? t('events.enabled') : t('events.disabled')}
            </Text>
          </Pressable>
        </View>
      </Card>
